...
namespace OverEasy
{
	partial class OverEasyGlobals
	{
        ...
        public static void LoadBillyStageDefValues()
        {
			...
			CreateStringSchema("EffectsFile", def.commonData.effect);
			CreateStringSchema("ParticleSystemsFile", def.commonData.particle);
        ...
        public static void UpdateBillyStageDef()
        {
			...
			objRaw.commonData = new StageDef.StageCommonData()
			{
				...
				SEBank7 = bank7File,
				effect = effectFile,
				particle = particleSystemsFile,
				objectData = objectGeometryFile,
		...
		private static void PopulateSetObjectsBilly(TreeItem activeNode)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				objNode.SetMetadata(2, (int)EditingType.BillySpawnPoint);
				...
				var animParentPath = modelNode.GetPath() + ":" + "Node_0";
			...
			if (loadedBillySetObjects != null && loadedBillySetObjects?.setObjs?.Count != 0)
			{
				...
				for (int i = 0; i < loadedBillySetObjects.setObjs.Count; i++)
				{
					...
					objNode.SetMetadata(1, i);
					//Node's object category
					objNode.SetMetadata(2, (int)EditingType.BillySetObj);
					//Attach a model instance
			...
			if (loadedBillySetDesignObjects != null && loadedBillySetDesignObjects?.setObjs?.Count != 0)
			{
				...
				for (int i = 0; i < loadedBillySetDesignObjects.setObjs.Count; i++)
				{
					...
					objNode.SetMetadata(1, i);
					//Node's object category
					objNode.SetMetadata(2, (int)EditingType.BillySetDesign);
					//Attach a model instance
